import type { FaqGroep } from "@/components/FaqAccordion";
import type { Locale } from "@/lib/i18n";

/**
 * Alle FAQ-content, per taal in 4 groepen (koop, huur, onderhoud,
 * accessoires). Losgetrokken uit de FAQ-pagina zelf zodat andere pagina's
 * (verkoop, verhuur) er een relevante deelgroep uit kunnen tonen zonder de
 * vragen dubbel te moeten onderhouden.
 */
export const FAQ_GROEPEN_NL: FaqGroep[] = [
  {
    id: "koop",
    label: "Te koop",
    vragen: [
...
export const FAQ_GROEPEN_FR: FaqGroep[] = [
  {
    id: "koop",
    label: "À vendre",
    vragen: [
...
export const FAQ_GROEPEN_EN: FaqGroep[] = [
  {
    id: "koop",
    label: "For sale",
    vragen: [
...
export function faqGroepenVoor(locale: Locale): FaqGroep[] {
  return locale === "fr" ? FAQ_GROEPEN_FR : locale === "en" ? FAQ_GROEPEN_EN : FAQ_GROEPEN_NL;
}

/** De vragen van één FAQ-groep ("koop", "huur", "onderhoud" of "accessoires") voor de opgegeven taal. */
export function faqVragenVoor(locale: Locale, groepId: string) {
  return faqGroepenVoor(locale).find((groep) => groep.id === groepId)?.vragen ?? [];
}
